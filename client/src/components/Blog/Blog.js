...
export default function Blog() {
  ...
  return (
    <div style={{ paddingBottom: posts ? "3%" : "100%" }}>
      <div
        style={{ display: "flex", marginLeft: "14%", marginBottom: "-30px" }}
      >
        <h1 style={{ textAlign: "left", padding: "15px", fontSize: "80px" }}>
          BLOG.
        </h1>
        <div style={{ paddingTop: "30px", paddingLeft: "43vw" }}>
          ...
          <Form.Control
            style={{ marginTop: user && user.is_admin ? 10 : 55, width: 200 }}
            name="value"
            value={search}
            onChange={(event) => {
      ...
      {posts ? (
        postsToDisplay.map((post) => {
          return <PostCard postContent={post} />;
        })
      ) : (
        <div class="center">
